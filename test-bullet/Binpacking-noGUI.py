...
class BinPackingEnv():    
    ...
    def __init__(self):
        ...
        self.actionFix = [[313, 24],[349, 155],[52, 132],[156, 67]]
        self.indexFix  = 0
    ...
    def cropImage(self, image, imageWidth, imageHeight, ldcWidth, ldcLength, fov, camDistance):    
        print("width, height, ldcwidth, ldclenght,fov,camDistance:", imageWidth, imageHeight, ldcWidth, ldcLength, fov, camDistance) 
    ...
    def getRandomAction(self):
        ...
        act = self.actionFix[self.indexFix]
        print("act:", act)
        self.indexFix = self.indexFix + 1
        return act
    ...
    def putBox(self, x, y, length, width, height, mass, xPix, yPix):   # xPix and yPix are the pixel location to be placed
        ...
        p.createMultiBody(mass, colBoxId, visualBoxId, [x + float(length)/2, y - float(width)/2, float(height)/2], [0, 0, 0, 1])
    ...
    def step(self):
        ...
        print("State shape:", np.shape(self.state))
    ...
    def render(self, mode='human', close=False):
        ...
        # GUI
        img_arr = p.getCameraImage(self.pixelWidth, self.pixelHeight, viewMatrix=viewMatrix, projectionMatrix=projectionMatrix, renderer=p.ER_TINY_RENDERER)       
        ...
        print("w,h,rbg0/1,dep0/1:", w, h, rgb[0], rgb[1], dep[0], dep[1])
        ...
        self.Y = size[0]
        self.X = size[1]
        print("depth_buffer_opengl=:", depth_buffer_opengl) 
        depth_opengl = self.far * self.near / (self.far - (self.far - self.near) * depth_buffer_opengl)
        print("camdistance:", self.camDistance)
        print("depth_opengl:", depth_opengl)
        ...
        actual_state_space = -depth_opengl
        test = depth_opengl.flatten()
        count = 0
        for ii, value1 in zip(range(1000),depth_opengl):
            for jj, value2 in zip(range(1000), value1):
                if value2 < 10:
                    print("ii,jj depth:", ii,jj, value2)        
                    count = count+1
        print("count in all:", count)
        self.state = actual_state_space 
        #print("render then state:", self.state); 
        print("in render the Max Height: ", np.max(self.state))
        return self.state 


#### Main Function ####
simrun = BinPackingEnv()
num_boxes = 2  ## can vary this according to your need
