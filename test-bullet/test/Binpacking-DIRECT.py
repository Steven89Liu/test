...
class BinPackingEnv():    
    ...
    def render(self, mode='human', close=False):
        ...
        rgb_tiny = np.reshape(img_arr[2], (self.pixelHeight, self.pixelWidth, 4))*1./255.
        # cv2.imshow("cropped Image RGB", rgb_tiny)

        depth_buffer_tiny = np.reshape(img_arr[3], [self.pixelHeight, self.pixelWidth])
 

        croppedImage2 = self.cropImage(depth_buffer_tiny, self.pixelWidth, self.pixelHeight, self.ldcWidth, self.ldcLength, self.fov, self.camDistance)
        ...
        actual_state_space = math.fabs(self.camDistance) - depth_tiny
