...
class HumanoidDoorEnv(gym.Env):
  ...
  def __init__(self, xml_path='/home/shjliu/workspace/EPR/bullet3-new/test-bullet/roomdoor',
               ...
               robot_init_pos=(0, 0, 200), door_init_pos=(3, 0, 0), goal_pos=(3, 0, 0),
  ...
  def step(self, action, debug=0):
    ...
    print("-------------------------------------------------------------------------")
    print("action=", action)
    print("joints index=", self.robot_joints_index)
    index_tmp = self.robot_joints_index.copy()
    index_tmp.sort()
    print("sorted=", index_tmp)
    print("joints index=", self.robot_joints_index)
    motorAngles = []
    for i in self.robot_joints_index:   
      jointState = pybullet.getJointState(self.body_name_2_joints['robot']['body_id'], i)
      motorAngles.append(jointState[0]) 
    print("curren=", motorAngles)
    print("action diff=", dict(zip(self.robot_joints_index, action - motorAngles)))
